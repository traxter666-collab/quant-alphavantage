#!/usr/bin/env python3
"""
Multi-Asset Trading System - SPX + QQQ + SPY
Complete three-asset options analysis with unified execution and comparison
"""
...
def run_multi_asset_analysis():
    """Run unified analysis for SPX, QQQ, and SPY"""
    print("COMPLETE MULTI-ASSET TRADING SYSTEM")
    print("Unified SPX + QQQ + SPY Options Analysis")
    print("=" * 80)
    ...
    try:
        ...
        # Generate comprehensive three-asset analysis
        print(f"\n" + "=" * 80)
        print("MULTI-ASSET COMPREHENSIVE COMPARISON")
        print("=" * 80)

        if spx_results and qqq_results and spy_results:
            # Extract key data from all three assets
            ...
            print(f"THREE-ASSET COMPARISON:")
            print(f"                    SPX              QQQ              SPY")
            print(f"  Price:           {spx_price:8.1f}        ${qqq_price:6.2f}         ${spy_price:6.2f}")
            print(f"  Consensus:       {spx_consensus:>12s}   {qqq_consensus_raw}/195 ({qqq_threshold_pct:.1f}%)   {spy_consensus_raw}/220 ({spy_threshold_pct:.1f}%)")
            print(f"  Direction:       {spx_direction:>12s}   {qqq_direction:>12s}    {spy_direction:>12s}")
            print(f"  Action:          {spx_action:>12s}   {qqq_action:>12s}    {spy_action:>12s}")
            ...
            if len(tradeable_assets) >= 3:
                best_opportunity = "ALL THREE ASSETS TRADEABLE"
                recommendation = "Multi-asset diversified approach - spread risk across SPX, QQQ, and SPY"
            ...
            # Direction correlation analysis across all three
            directions = [spx_direction, qqq_direction, spy_direction]
            unique_directions = set(directions)

            if len(unique_directions) == 1:
                correlation_status = "PERFECT ALIGNMENT"
                correlation_note = "All three assets showing same directional bias - strong market consensus"
                ...
                correlation_status = "FULL DIVERGENCE"
                correlation_note = "All assets showing different directions - high uncertainty environment"

            print(f"\nCORRELATION ANALYSIS:")
            print(f"  Three-Asset Correlation: {correlation_status}")
            ...
            if len(tradeable_assets) > 1:
                ...
                print(f"  Multi-Asset Approach: Spread 2-3% across {len(tradeable_assets)} assets")
                print(f"  Individual Allocation: 0.8-1.2% per asset for diversification")
                print(f"  Correlation Benefit: Reduced single-asset concentration risk")
        ...
        multi_asset_summary = {
            ...
            'analysis_type': 'COMPLETE_MULTI_ASSET_SPX_QQQ_SPY',
        ...
        overall_success = spx_success or qqq_success or spy_success
...
def generate_trading_plan():
    ...
    try:
        ...
        print(f"Tradeable Assets: {len(results.get('tradeable_assets', []))}/3")
        ...
        if results.get('tradeable_assets'):
            ...
            print(f"3. Position Sizing: 1.0-1.5% per asset for diversification")
            ...
            print(f"  All three assets below threshold")
        ...
        print(f"  Per-Asset Limit: 1.5% maximum")
        print(f"  Exit Triggers: 75% threshold breach on any asset")
        print(f"  Correlation Risk: Monitor for simultaneous moves")
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("SPX + QQQ + SPY analysis complete")
        ...
        if plan_success:
            ...
            print("Complete three-asset trading recommendations available")
